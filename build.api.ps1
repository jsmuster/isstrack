...
$configSource = Join-Path $projectDir "config_prod.json"
$configTarget = Join-Path $projectDir "config.json"

$noCacheFlag = if ($args -contains "--force" -or $args -contains "--no-cache") { "--no-cache" } else { $null }

if (-not (Test-Path $configSource)) {
    Write-Host "[Build] ERROR: Missing config file: $configSource" -ForegroundColor Red
    exit 1
}

Copy-Item -Path $configSource -Destination $configTarget -Force
Write-Host "[Build] Using production config: $configSource" -ForegroundColor Cyan
